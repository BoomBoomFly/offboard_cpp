...
TEST(MissionController, LocalizationLossLandsInPlaceAndResetMovesHome)
{
  ...
  ++inputs.now_us;
  controller.tick(inputs);
  EXPECT_EQ(controller.state(), offboard_cpp::MissionState::COMPLETE);
}

TEST(MissionController, CancelDuringHoverReturnsLocalHome)
{
  offboard_cpp::MissionConfig config;
  ...
  ++inputs.now_us;
  controller.tick(inputs);
  EXPECT_EQ(controller.state(), offboard_cpp::MissionState::HOVER);

  inputs.cancel_requested = true;
  ++inputs.now_us;
  const auto actions = controller.tick(inputs);
  EXPECT_EQ(controller.state(), offboard_cpp::MissionState::RETURN_LOCAL);
  EXPECT_EQ(controller.state_reason(), BOOMBOOM_STATE_REASON_CANCELLED);
